...
#         except Exception:
#             await edit_delete(zedevent, "`unblock `@tgscanrobot` and then try`")
#         response = await conv.get_response()
#         await event.client.send_read_acknowledge(conv.chat_id)
#         await zedevent.edit(re
...
async def count(event):
    ...
    b = 0
    result = ""
    zzz = await edit_or_reply(event, "**⪼ جـارِ جلب إحصائيـات حسـابك . . .༗**")
...
async def _(event):
    ...
    try:
        ...
        await edit_or_reply(event, "**⎉╎تم تفعيل وضع تقييد المحتوى .. بنجـاح ✓**")
